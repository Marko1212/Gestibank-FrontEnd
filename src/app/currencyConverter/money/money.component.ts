...
export class MoneyComponent implements OnInit, OnDestroy {
  ...
  @Output()
  updateExchangeRates: EventEmitter<ConversionRate> = new EventEmitter<
    ConversionRate
  >();
  ...
  @ViewChild(NgModel, { static: false }) moneyInputModel: NgModel;
  ...
  ngOnInit() {
    ...
    this.currentCurrencyCodeSubject = new BehaviorSubject<string>(
      ...
      .pipe(
        ...
        switchMap((dataArray) =>
          this.conversionRatesService.getLatestExchangeRates(
            dataArray[0],
            dataArray[1],
            this.otherCurrencyCodes.filter(
